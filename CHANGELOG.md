...
- **无编号图片按文件修改时间排序。** 当文件夹中没有符合编号规则的图片时，自动回退为按文件修改时间升序排列（最旧的文件排在最前面），无需重命名文件即可生成 PPTX。
- **交互引导放宽输入限制。** 拖入终端引导不再要求文件夹必须包含编号规则图片，只要有 JPG/PNG 图片即可接受。

### 变更

- `has_numbered_images()` 替换为 `has_any_images()`，交互模式和命令行模式均同步更新。
- 互动横幅文案和帮助文本更新，标明回退排序规则。
